...
for p in (CANDS, FIGS): p.mkdir(parents=True, exist_ok=True)
...
def step_score(top, s1_db: Path, dem30: Path, out_csv: Path):
    ...
    if dem_sh != s1_sh or dem_crs != s1_crs or dem_tr != s1_tr:
        ...
        reproject(source=dem, destination=dem_res,
                  src_transform=dem_tr, src_crs=dem_crs,
                  dst_transform=s1_tr,  dst_crs=s1_crs,
    ...
    for i, r in top.reset_index(drop=True).iterrows():
        mask = geometry_mask([r.geometry.__geo_interface__], out_shape=s1_sh, transform=s1_tr, invert=True)
        pix = int(mask.sum())
        if pix == 0:
            rows.append({"idx": i+1, "area_ha": float(r.get("area_ha", np.nan)), "pix": 0, "frac_ok": 0.0})
            continue
        ok = (s1 > 0.5) & (rel <= 5.0) & mask & np.isfinite(s1) & np.isfinite(rel)
        rows.append({"idx": i+1, "area_ha": float(r.get("area_ha", np.nan)), "pix": pix,
...
def step_render_figs(top, alos_rgb: Path, s1_rgb: Path|None, s1_db: Path, buffer_m: int):
    ...
    for i, r in top.reset_index(drop=True).iterrows():
        ...
        if rdA is not None and rdA.count >= 3:
            ...
            l, rgt, b, t = window_extent(rdA, w)
            A = rdA.read([1,2,3], window=w).astype("float32")
            A = np.clip(A / (np.percentile(A, 99) + 1e-6), 0, 1)
            ax[0].imshow(np.transpose(A, (1,2,0)), extent=(l, rgt, b, t))
            ax[0].set_title("ALOS-2 Δ (colorized)")
        else:
            ax[0].text(0.5, 0.5, "ALOS-2 Δ not found", transform=ax[0].transAxes, ha="center", va="center")
        ...
        plt.close(fig)
        print("  ->", out)
    if rdA is not None: rdA.close()
    if rdSRGB is not None: rdSRGB.close()
    rdSDB.close()
...
def main():
    ...
    step_render_figs(top, alos_rgb=alos_rgb, s1_rgb=s1_rgb if s1_rgb.exists() else None,
                     s1_db=s1_db, buffer_m=args.buffer_m)
